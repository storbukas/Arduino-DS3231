 ...
 *  DS3231.cpp
 *
 *  Class file for the DS3231 Real-Time Clock
 *
 *  created 03 Feb 2021
...
const uint8_t daysArray[] PROGMEM = {31, 28, 31, 30, 31, 30,
                                     31, 31, 30, 31, 30, 31};
...
uint8_t DS3231::isReady(void) { return true; }
...
uint8_t DS3231::bcd2dec(uint8_t bcd) { return ((bcd / 16) * 10) + (bcd % 16); }

uint8_t DS3231::dec2bcd(uint8_t dec) { return ((dec / 10) * 16) + (dec % 10); }
...
bool DS3231::isLeapYear(uint16_t year) { return (year % 4 == 0); }
...
uint8_t DS3231::dow(uint16_t y, uint8_t m, uint8_t d) {
  ...
  y -= m < 3;
  dow =
      ((y + y / 4 - y / 100 + y / 400 + pgm_read_byte(dowArray + (m - 1)) + d) %
       7);
